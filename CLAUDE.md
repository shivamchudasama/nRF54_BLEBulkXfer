...
- `_ASW/` — application software. `main.c` plus one folder per module:
  ...
  - `_DATA_STORE/` — BulkXfer Server callbacks, the 64 KiB segment buffer (`DS_BUF_SIZE`) and a low-priority dump thread that prints `0xADDR: xx …` lines, paced by `log_buffered_cnt()` so the deferred log never drops lines. Sends RESULT/STORED short messages. `gi_DataStore_Init()` runs in `main()` before advertising.
